...
const baseUrlVentas = import.meta.env.VITE_API_URL_VENTAS || "http://localhost:8080";
const baseUrlDespachos = import.meta.env.VITE_API_URL_DESPACHOS || "http://localhost:8081";
...
export const FormDespacho = ({ venta, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(
        `${baseUrlVentas}/api/v1/ventas/${venta.idVenta}`,
        jsonDataSales,
        {
          headers:{
      ...
      await axios.post(`${baseUrlDespachos}/api/v1/despachos`, jsonData, {
        headers:{
          'Content-Type': 'application/json',
          'Accept': 'application/json'
    }
